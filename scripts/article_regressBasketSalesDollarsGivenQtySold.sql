...
create temp table addBasketDollars as (
SELECT BasketKey, businessdate, dow,
sum(retailamount) as basket_retailamount,
sum(RetailQuantity) as basket_RetailQuantity
...
b.basket_retailamount, b.basket_RetailQuantity,
a.retailamount, a.RetailQuantity

FROM distincts_00 a
left join addBasketDollars b on (a.BasketKey=b.BasketKey)
);

create temp table articleDat2 as (
SELECT *, basket_retailamount-retailamount as basket_retailamount_exclArticle
...
avg(basket_retailamount_exclArticle) as mean_y,
corr(RetailQuantity, basket_retailamount_exclArticle) as corr_xy,
stddev(RetailQuantity) as sd_x,
stddev(basket_retailamount_exclArticle) as sd_y,
...
'RetailQuantity' as x, 'basket_retailamount_exclArticle' as y,
...
CREATE OR REPLACE TABLE  `gcp-wow-finance-de-lab-dev.price_elasticity.regress_article_qty_basketSalesDollars` as (
select *,
concat("between ",from_date," and ",to_date," for article ",article,"(",RetailUOM,"), for ",dow,"s, given ", n, " baskets containing this article, the Sales $s for baskets containing this article move by ", round(slope,3)," for each increase in RetailQuantity.") as description
from finalDat
);
